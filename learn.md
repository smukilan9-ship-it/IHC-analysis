...
## 2. What immunohistochemistry is
...
> "In this image, how many cells were found, and how many are positive for the
> selected marker?"
...
The current Quantification starting values are:
...
## 4. Segmentation validation and what it does not prove

The project separates **segmentation validation** from **marker-label validation**.
...
F1 is low if either precision or recall is low. That is why it is useful when both false detections and missed detections matter.
...
The full external audit reports a micro-F1 of **0.776** across 268 HNSCC
tiles.

**Micro-F1** pools all true positives, false positives, and false
negatives across the dataset first, then computes one F1 score.
...
The full external audit reports:
...
## 5. TIM-3 membrane remeasurement, what was tried, and why it was disabled
...
## 6. The tempting question, and why serial sections cannot answer it
...
> Population-level spatial association can be studied.
...
## 7. MNN, why it sounded useful, and why it failed
...
## 8. Spatial association as the serial-section alternative
...
- The test asks whether the populations are closer than expected under a suitable chance
  model.
...
## 9. Spatial association workflow
...
Marker-positive cells are exactly what is being tested, so they should not guide
registration.
...
> "If this point were not allowed to help with the alignment, could the alignment still
> predict it?"
...
## 10. Cross-type Ripley K and L(r)-r

After registration and masking, there are:

- a set of A-positive cell centers,
- a set of B-positive cell centers,
- a certified tissue window.

Now the question is whether A and B are unusually near each other.
...
## 11. DCLF, the one p-value for the whole curve

A curve creates a statistical trap. If every radius is tested separately and the most exciting one is then picked, false positives inflate.
...
Under a synthetic true null, the DCLF false-positive rate was
**0.045** and the mean p-value was **0.515**. That is the goal: a fair test
should produce p < 0.05 about 5% of the time under a true null, and p-values
should average around 0.5.

---

## 12. The null model problem
...
The false-positive rate was **1.00** under shared preference for the
...
The false-positive rate was **0.87** under shared preference. That is
far too high.
...
The false-positive rate was **0.85** under shared preference. That is
also far too high.
...
> "If two populations are close only because both prefer the same tissue region, that
> closeness should not count as strong evidence. Pairs in already-dense regions are
> down-weighted, asking whether proximity remains after each population's own
...
## 13. Calibration and demonstration

### 13.1 Calibration selected the production configuration

A 500-run calibration reports:
...
The validation includes a locally certified demonstration:
...
## 14. When true co-expression is possible
...
## 15. Restained same-cell workflow

The restained workflow is:
...
This avoids the serial-section problem because the analysis is no longer trying to match
...
The pipeline offers optional hematoxylin preprocessing:
...
## 16. The 2×2 same-cell table and its statistics
...
The Case1_M1_0_0 nuclear demonstration table is:
...
> "If CD8 and FOXP3 positivity were unrelated in this table, about
> 1.28 double-positive cells would be expected. 11 were observed."
...
For this table, φ = 0.442, a moderate positive association.
...
> "When many tiles are tested, some may look significant by luck. BH-FDR adjusts the
...
The project labels the Case1_M1_0_0 result as a **current nuclear demonstration** and
...
## 17. Restaining validation and what it means

### 17.1 Three-tile segmentation pilot

A three-tile pilot from HNSCC Case 1 reports:
...
The full audit across 268 tiles and eight patients reports:
...
mIF foreground had higher AEC OD in most tiles:
...
> "If one foreground pixel and one background pixel are picked at random, how often
...
Against a non-expert mIF intensity proxy:
...
This document ends by separating validation layers. This is essential because one
...
The dataset map separates purposes:
...
> How many double-positive cells would be expected if the two markers were
> independent.
...
> This is an analysis pipeline that counts stained cells, compares spatial
> neighborhoods carefully, and shows when the data is not strong enough for a
