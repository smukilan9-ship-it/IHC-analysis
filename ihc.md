...
>
> **Status (2026-06-22):** validated *method* and working *instrument*; **not** a finished
> external biological claim (no CD8/TIM-3 pair has certified — a data limit, not a code limit).
> Defensible for internal / method-development use. See §12 and Appendix A.
>
> **Reading guide — the correction convention.** Where an earlier method was later shown
> wrong, the original text is **kept** and marked with a `CORRECTION`/`SUPERSEDED` banner that
> points to the section with the current method. This is deliberate: the failures are part of
> the record. All such corrections are also collected in **Appendix B — Corrections &
> supersessions log**. The current method is always the one stated in the banner / Appendix B.

A technical document describing the entire current state of the IHC
Analyzer: a desktop + CLI tool for automated immunohistochemistry analysis of
H‑DAB brightfield tissue images. It has two independent pipelines:
...
claim. True single‑cell co‑expression requires **multiplex imaging on one
section** (CyCIF, CODEX, mIF, Xenium) — out of scope here and noted as future
work.
...
```text
   ...
   Raw IHC images ───► │  QuPath (headless) + InstanSeg │  nucleus segmentation
        ...
        │                            │        cross_k_all_nulls (3 nulls+DCLF), A∩B mask
...
Validated against manual counts at roughly **~90% agreement** on the
development cohort. Confidence is flagged LOW when `total < 50`, positivity
`< 0.1%`, or `> 95%`.
  ...
  "spatial_association": {
    ...
    "association": {
      "CD8__TIM3": {                       // neutral key — NOT "CD8+TIM3+"
        ...
        "primary_null": "inhomogeneous",
        // top-level null_*/global/p_values mirror the PRIMARY (inhomogeneous) null
        "null_lower_L": [...], "null_upper_L": [...], "global": { ...DCLF... },
        "nulls": {
          "homogeneous":   { "null_lower_L": [...], "p_values": [...],
                             "global": { "significant": false, "global_p_dclf": 0.31,
                                         "direction": "none", ... } },
          "inhomogeneous": { "bandwidth_um": 50.0, "bandwidth_method": "fixed_um",
                             "bandwidth_sensitivity": { "0.5x": {...}, "1x": {...}, "2x": {...} },
                             "global": { "significant": true, "direction": "association", ... } },
          "toroidal":      { "global": { "significant": true, "direction": "association", ... } }
        },
        "robustness": {
          "verdict": "robust",            // robust | csr_only | none | mixed
          "direction": "association",
          "summary": "Robust: significant association under BOTH structure-preserving nulls ...",
          "per_null_significant": { "homogeneous": true, "inhomogeneous": true, "toroidal": true }
        },
...
Population A is always held fixed; population B is repositioned under **three
different null models**, reported side by side so robustness to the choice of null
is visible. Each null yields the same outputs as before — null mean, **95%
envelope** (2.5/97.5 percentiles) in K and L−r space, per‑radius p‑values, and the
**directional DCLF global p** (below).

1. **Homogeneous CSR** (the original null). B uniform‑random in the window
   (rejection sampling via `shapely.contains_xy`). **Weakest** null: it cannot tell
   a real cross‑type interaction from two populations that *independently* prefer
   the same tissue compartment (e.g. inflamed stroma) — shared first‑order
   intensity reads as "association". Kept only as a baseline.
2. **Inhomogeneous (Kinhom)** — the literature standard for "association beyond
   first‑order intensity". B is resampled from **its own estimated intensity
   surface** (grid Gaussian‑KDE of B's positions, masked to the window), so the
   null preserves B's tissue preference and the test asks whether the *cross*
   relationship exceeds what each population's individual preference already
   produces. The KDE **bandwidth** is explicit: default **50 µm** (the top of the
   DCLF band — preserves compartment‑scale structure ≥ the interaction scale while
   leaving within‑band interaction detectable). A **0.5×/1×/2× sensitivity sweep**
   is reported so the result's robustness to bandwidth is visible. *(Scott's rule
   was rejected as the default — it is driven by global point spread and
   over‑smooths a multimodal intensity toward uniform, collapsing Kinhom back to
   CSR.)*
3. **Toroidal shift** — preserves B's **entire real pattern** (its actual
   clustering, not a smoothed estimate) and randomizes only B's position relative
   to A via random wrap‑around translations on the window's bounding box. Fewer
   free parameters than Kinhom and robust to bandwidth choice; assumes the pattern
   is **stationary** over the rectangular window (the standard toroidal
   assumption). Controls for shared preference when that preference is
   approximately stationary (distributed compartments); Kinhom is the more general
   guard against strongly non‑stationary intensity.
...
destroys the per‑section intensity that is the whole comparison. All three nulls
instead reposition one population while holding the other fixed.

**Robustness reporting.** A `robustness` verdict combines the per‑null DCLF calls:
`robust` (significant, same direction, under **both** structure‑preserving nulls →
a real signal beyond shared tissue preference), `csr_only` (significant **only**
under homogeneous CSR → **critical**: no association beyond shared preference —
reported honestly, never hidden), `none`, or `mixed`. The headline interpretation
states whether the conclusion holds across nulls. For backward compatibility the
**inhomogeneous** (primary) null's curves are also copied to the result's top
level, so the plot/QC/UI keep working with the rigorous null.
...
**Conditions — matched to the production pipeline.** The run uses the *same*
statistic and the *same* settings as `spatial.run_spatial_association`:
...
- **Radii** `0–100 µm` in `2 µm` steps, converted to px exactly as the pipeline
  does; **`n_perm=1000`, `seed=0`**; the per‑spot verdict is the pipeline's own
  direction‑resolved **global DCLF** call on the 10–50 µm band (not a per‑radius
  OR).
- **Tissue mask — a deliberate, documented difference.** The pipeline derives its
  ...
  with no brightfield image to threshold. The area and the CSR
  null are bound with the **convex hull of all cells in the spot**. This is an intentional
  substitution for the *image front end only*: what is validated is the **statistic**
...
### (c) Null‑model validation — `validation/validate_null_models.py`
Three constructed point patterns prove each null behaves correctly:
...
The **decisive** row is the first: homogeneous CSR **false‑positives** on shared
tissue preference, while both structure‑preserving nulls correctly report **no**
association (verdict `csr_only`) — proving the structure‑preserving nulls remove
the first‑order‑intensity bias. The bandwidth sensitivity sweep is also exercised:
at 2× the default bandwidth Kinhom begins to over‑smooth (toward CSR) and
false‑positives on the shared‑preference case, which is exactly why the default is
fixed at the compartment scale and the sweep is reported.
...
| **Multiple nulls, reported together** | **A single null** (homogeneous CSR) | No single null is unimpeachable. Homogeneous CSR alone is too weak (shared tissue preference → false association). **(Design update — §15.3:** the original three‑null robustness vote was itself anti‑conservative under shared preference and is **retired**; production now uses a size‑controlled **reweighted‑primary** null with homogeneous CSR kept only as a shared‑preference flag. The principle below still holds — a result that appears *only* under CSR is flagged `csr_only`; what changed is which null gates the verdict.) |
| **Fixed µm KDE bandwidth** (Kinhom default 50 µm) + sensitivity sweep | **Scott's/Silverman's data‑adaptive rule** | The adaptive rule is driven by the *global spread* of the points and over‑smooths a **multimodal** intensity (many small tissue compartments) toward uniform, collapsing Kinhom back into homogeneous CSR. A fixed bandwidth at the compartment scale preserves the right structure; the 0.5×/1×/2× sweep documents robustness. |
...
|---|---|
| "CD8⁺ and TIM‑3⁺ populations are spatially associated up to *r* µm (DCLF p<0.05) **robustly across homogeneous CSR, inhomogeneous, and toroidal nulls** — i.e. beyond shared tissue preference." | "These cells co‑express CD8 and TIM‑3." |
...
| "Population‑level cross‑type Ripley's K / g(r), n=1000 permutations × 3 null models, global DCLF envelope test." | "X% of CD8⁺ cells are TIM‑3⁺ double‑positive." |
...
- **Toroidal null assumes stationarity** over the window; under strongly
  *non‑stationary* shared preference it is less robust than Kinhom — which is why
  both structure‑preserving nulls are reported and a `csr_only`/`mixed` verdict is
  surfaced rather than a single number.
- **Inhomogeneous null depends on KDE bandwidth**; the default is fixed at the
  compartment scale and a 0.5×/1×/2× sensitivity sweep is reported, but an
  adversarial intensity could still be mis‑estimated.
...
- **True single‑cell co‑expression** (one‑section multiplex) is **future work**,
  not implemented.
...
| `default_pixel_size`, `pixel_overrides` | pixel size + per‑image overrides |
...
breaks a prior claim beats a flattering null that cannot be defended.

---

## 16. Cross-validation against spatstat (R reference)

The intensity-reweighted inhomogeneous cross-K is a custom Python implementation,
so it was cross-validated against the reference implementation in **spatstat**
...
**Stage A — intensity surface.** The `_loo_kernel_intensity` (leave-one-out
...
explained by the 4h Gaussian-tail truncation (exp(-8) ~ 3.4e-4); it is a controlled
approximation, not a bug (widening the cutoff to 6h would drive it to ~1e-8).

**Stage B — the estimator (the decision gate).** The `_cross_k_inhom_weighted`
...
The estimator equals spatstat's uncorrected Kcross.inhom to **floating-point
...
by 5-74%, which **confirms** the estimator is the uncorrected ("none") one, as
...
**Stage C — the test.** Not forced to match numerically: the null is a
...
procedure, not an estimator bug. The null's calibration and power are validated
...
confirmed the uncorrected estimator is computed correctly; this section settles
...
**Method.** A translation (Ohser) edge correction was added — exact for a
rectangular window, weight e = |W|/((Lx−dx)(Ly−dy)) per pair — applied through the
SAME function to observed and null K (symmetric by construction). The translation
correction agrees with spatstat's `correction="translate"` to ~4e-3 (radius-
dependent; a minor normalization nuance in spatstat's variant — the "none" matches
spatstat at 1e-15, §16), and it is a substantial correction (weights up to ~1.25×
near the boundary). The full §15 three-regime calibration was re-run (500 reps,
...
decisive test is translation, which was run.
...
the symmetric DCLF null, it provably changes nothing.** No production code changed.
...
- **Landmark-driven transform.** A least-squares **similarity** (rotation + uniform
  ...
  the downstream cross-K metric stays valid — the method deliberately does **not** non-rigidly
...
### 19.4 Quantification tab — biomarker thresholds; membrane trial retired
The Quantification UI exposes required stain identity and optional per-marker filename
tokens/DAB thresholds. It previously exposed membrane/cytoplasm-ring measurement, but
that control was removed after the TIM-3 trial produced 4,433 positives (32.0%) versus
the original QuPath result of 61 (0.44%). The backend experiment remains in
`cell_expansion.py`; visible Quant and Spatial workflows now force cytoplasm measurement
off and preserve QuPath classification until a held-out manual validation succeeds.
...
- **Verdict renamed + 4 states.** The old `NON-CORRESPONDING` overclaimed (failing to
  ...
  to that ROI — field-wide accuracy is **not** claimed where no landmark exists.
- **LOO honesty fix.** Leave-one-out is *fit-unbiased* (a point never sits in the
  ...
  annotator, this is LOO-grade validation, reported as such — not a gold standard.
...
### 19.6 Recorded future work (NOT implemented — deferred by request)
The following Spatial-Association feature work is intentionally deferred and recorded
here as future features (no code written this round):
- **In-app landmark picker in the Spatial tab** — promote the standalone blinded
  `landmark_tool.html` + `phase_a_finalize.py` flow into the UI: per-pair point
  placement → live held-out TRE + 4-state verdict badge + coverage → certified /
  locally-certified pairs feed Phase B automatically; failed pairs show the reason;
  forced overrides watermarked and excluded from certified statistics.
...
(1) can be CERTIFIED by the §19 landmark-driven pipeline and (2) support cross-type spatial
association, then run Phase B end-to-end on anything that certifies — the 052526 pairs
...
| dataset | modality / markers | landmark GT | license | access | verdict |
|---|---|---|---|---|---|
...
**True-multiplex CD8 + TIM-3 (gives the biology but BYPASSES the §18-19 registration):**
...
≤ 5 µm on deformed pairs — is exactly what is observed:
...
This is a **data-access** limit, not a method failure, and no cell detector was fabricated
...
substitute for a dedicated **re-stained CD8/TIM-3** acquisition, which (per §19.5) removes the z-gap
...
now **enforces** that discipline: no spatial result is presentable as certified, the
Restained path is fail-closed on correspondence, provenance is complete, and stale
descriptions are corrected. Defensible for **internal / method-development** use. The
blocking gap to external use is Stage 3 (in-app certification) plus genuinely
registrable data (ideally restained same-section), as already documented in §19.6 and
§20.7.

---

## Appendix A — Validation ledger (consolidated)

Every validation run, organised by type. Source of record:
`outputs/ihc_validation_results_by_type.xlsx` (15 sheets) and `ihc_validation_results_all.csv`.
Each row states the dataset, what is established, and — critically — what is *not*.

| Validation type | Dataset | Headline result | What it establishes / what it does NOT |
|---|---|---|---|
| **Cross-K checks** | Synthetic, directly calculable | Fast `cKDTree` cross-K equals brute force **exactly** | Core pair-counting is numerically correct. |
| **Spatstat cross-validation** | Synthetic CSR/shared/calibration + 1 real Schürch CODEX spot | Max L-curve rel. diff **~10⁻¹⁴** (synthetic) → **1.4×10⁻¹⁰** (real); K-ratio exactly 1 | The estimator equals R `spatstat` to floating point. Estimator only — the bootstrap null is separate. |
| **DCLF tests** | Synthetic null / clustered / separated | Null FP **0.045**, mean p 0.515; clustered & separated **p≈0.002** correct direction | The single global significance call is calibrated. |
| **Null crisis** | 500 shared-preference realisations (no interaction) | Retired nulls FP: homog-CSR **1.00**, smoothed-inhom **0.87**, toroidal **0.85** | The old three-null "robust" design was anti-conservative — measured, not assumed. |
| **Reweighted primary** | 500-run calibration (shared / uniform / 7µm / 25µm) | FP **0.032** (shared) / **0.064** (uniform); power **1.000** @7µm, **0.992** @25µm; 75µm bw + LOO | Production null controls error near nominal with high power. Assumes architecture coarser than 75µm. |
| **Edge correction** | Paired 500-run, uncorrected vs translation | FP **0.032 identical** either way; power unchanged | The uncorrected estimator is the right choice under the symmetric DCLF null. |
| **CODEX controls** | Schürch CRC CODEX — 258,385 cells, 140 spots | CD8↔CD4 assoc. **20/40** spots; CD8↔Treg **3/31**; CD8↔tumour segreg. **9/40**; many CSR-only signals vanish | Real-data biology in direction; calibrated null is stricter and *less* flattering than CSR. |
| **Registration engine** | Synthetic known shift | Recovered to **~0.1 px**, structural NCC **0.994** | Transform estimation is correct under true correspondence — does not certify any real pair. |
| **Registration public** | ANHIR/CIMA expert landmarks — 83 pairs, 9 series | **3** locally certified, **80** not certifiable, **0** globally certified; scrambled control rejected | Certification logic admits only measured spatial support on real expert landmarks. |
| **Registration 052526** | Target CD8/TIM-3 cohort — 7 serial-section FOV pairs | **None certified** (held-out TRE ~38–92 µm vs 5 µm tol.) | The target cohort cannot support a 10–50 µm spatial claim. A data limit, not a method failure. |
| **Spatial demo (end-to-end)** | CIMA/ANHIR lung-lesion_1 Ki67↔proSPC, certified ROI | Homog-CSR **p=0.001 (sig.)**, calibrated primary **p=0.325 (n.s.)** → shared-preference artifact | The full serial pipeline runs on real data and the calibrated null refuses the CSR false positive. |
| **Restained pilot** | HNSCC v2 — 3 Case-1 tiles, expert masks | Raw F1 **0.854**, preprocessed **0.847** | Preprocessing is a recall/precision trade-off, not a general accuracy gain. |
| **Restained nuclear demo** | HNSCC v2 — Case1_M1_0_0 (CD8 measured in *nucleus* — wrong compartment) | 51 CD8⁺ / 11 FOXP3⁺ / **11 double⁺**, Fisher p≈**1.85×10⁻¹¹**, φ≈**0.442** | Pipeline mechanics work — and the *wrong* compartment fabricates a strong false co-expression. |
| **Restained external** | HNSCC v2 — 268 tiles, 8 patients; 264 complete bundles | Nucleus seg. micro-F1 **0.776**; CD8 AEC pixel-AUC strong, FOXP3 variable; correct-compartment Case1 = **0 double⁺**, p=1 | End-to-end on the full public cohort; segmentation honest; no expert marker labels → co-expression not biologically validated. |

**One-line reading:** the *statistics and certification machinery* are validated (synthetic,
real-data, and against the R reference); the *biological CD8/TIM-3 claim* is not, because the
target data does not certify and marker thresholds lack expert ground truth.

---

## Appendix B — Corrections & supersessions log

Consolidated record of every place an earlier method/claim was superseded. The original text is
retained inline (marked with a banner) for honesty; this is the single index of what changed.

| ID | Original claim (inline §) | Status | Current method / correction |
|---|---|---|---|
| **A2** | "~90% manual-count agreement" for quantification (§3) | **WITHDRAWN — UNVERIFIED** | No shipping artifact backs it. Quant agreement is unverified pending a manual-count F1/κ. The HNSCC F1 0.776–0.847 (§21.3) is a *different* endpoint (nuclear segmentation), not DAB quant. |
| **A3** | "Three null models" robustness vote as production primary (§7, §6 schema) | **RETIRED** | Anti-conservative under shared preference (Null-crisis: 85–100% FP). Replaced by the **intensity-reweighted inhomogeneous cross-K** primary + homogeneous-CSR as a diagnostic baseline only (§15.3–15.4). |
| **A4** | §9b real-data table figures | **SUPERSEDED** | That table used the retired CSR null. Production-primary (reweighted) re-run gives lower, more honest numbers — see §15.7 and Appendix A (CODEX controls). |
| **A5** | `_reweight_run*.log` artifacts | **SUPERSEDED** | Stale logs retained with "SUPERSEDED — DO NOT CITE" banners; cite `reweighted_null_output.txt` / Appendix A instead. |
| **A6/A7** | Provenance completeness | **RESOLVED** | Results now stamp `reweight_bandwidth_um` (75), `null_seed` (0), and the architecture-scale assumption (`architecture_scale_measured:false`). |
| **A8** | Cohort multiple-testing | **RESOLVED** | Benjamini–Hochberg FDR across per-pair DCLF p-values, written to `spatial_cohort_fdr.json`. |
| **Changelog §14.0** | Three-null + intersection window + QC pivot | **PARTIALLY SUPERSEDED** | The three-null portion was retired (see A3 / §15.3); the intersection window and fail-closed registration QC remain current. |

*Method-development note: the remaining open items (a CERTIFIED CD8/TIM-3 pair; a data-backed
quantification agreement number; measuring architecture scale per-image) are tracked in
`problems.md`. They are data/validation gaps, not unresolved code defects.*
