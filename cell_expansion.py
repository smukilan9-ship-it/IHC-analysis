...
def _dab_od_channel(rgb: np.ndarray, stain_vectors: dict, background) -> np.ndarray:
    """
    Per-pixel DAB optical density via colour deconvolution, QuPath convention:
        OD = -log10((I + 1) / background)   (per channel)
        DAB = OD · inv(normalized_stain_matrix)[:, DAB]
    Returns an HxW float32 array directly comparable to QuPath's "DAB: Mean".
    """
    ...
    M = np.array([_norm(stain_vectors["hematoxylin"]),
                  _norm(stain_vectors["dab"]),
                  _norm(stain_vectors["residual"])])       # rows = stain vectors
    dab = od @ np.linalg.inv(M)[:, 1]                      # HxW
...
def _mask_stats(geoms, dab_od, x0, y0, w, h, want_values=None):
    """
    ...
    vectorized point-in-polygon test, and return a per-geom dict:
        {"mean": float|None, "p90": float|None, "n": int, "values": np.ndarray|None}
    ...
    `values` (the raw per-pixel OD array) is returned only for geometry indices in
    `want_values`, so the caller can compute a positive-pixel fraction at an
    arbitrary pixel threshold without re-rasterizing.
    """
    import shapely
    ...
    for idx, g in enumerate(geoms):
        if g is None or g.is_empty:
            out.append({"mean": None, "p90": None, "n": 0, "values": None})
            continue
        ...
        if not n:
            ...
            continue
        vals = sub[mask]
        keep = (vals.astype(np.float32)
                if (want_values is not None and idx in want_values) else None)
...
def measure_cytoplasm_dab(
    ...
    *completeness* feature that separates a faint concentrated arc (real positive)
    ...
    array as `"ring_values"` (used by the threshold-tuning harness to sweep cutoffs).
    """
    ...
    H, W   = rgb.shape[:2]
    dab_od = _dab_od_channel(rgb, stain_vectors, background)
    del rgb
    ...
    raw_ring_vals = {}          # feature idx -> raw (uncalibrated) ring pixel OD array
    ring_fallbacks = 0
    ...
    if membrane_pix_thr is not None:
        ...
        for i, r in enumerate(results):
            ...
            if r and vals is not None and len(vals):
                r["membrane_pos_frac"] = round(float((vals > raw_thr).mean()), 5)
    if keep_ring_values:
        for i, r in enumerate(results):
            vals = raw_ring_vals.get(i)
